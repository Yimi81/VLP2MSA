...
def set_seed(config):
    ...
    os.environ['PYTHONHASHSEED'] = str(config["trainer"]["seed"])
    np.random.seed(config["trainer"]["seed"])
    torch.manual_seed(config["trainer"]["seed"])
    torch.cuda.manual_seed(config["trainer"]["seed"])
    torch.cuda.manual_seed_all(config["trainer"]["seed"])  # if you are using multi-GPU.
...
def run():
    ...
    data_loader, valid_data_loader, test_data_loader = init_dataloaders(config, module_data)
...
def init_dataloaders(config, module_data):
    ...
    if "type" in config["data_loader"] and "args" in config["data_loader"]:
        ...
        config['data_loader']['args'] = replace_nested_dict_item(config['data_loader']['args'], 'tsfm_split', 'val')
        valid_data_loader = [config.initialize("data_loader", module_data)]

        config['data_loader']['args'] = replace_nested_dict_item(config['data_loader']['args'], 'split', 'test')
        config['data_loader']['args'] = replace_nested_dict_item(config['data_loader']['args'], 'tsfm_split', 'test')
        ...
        for dl_cfg in config['data_loader']:
            ...
            new_cfg_li.append(dl_cfg)
        config._config['data_loader'] = new_cfg_li
        test_data_loader = [config.initialize('data_loader', module_data, index=idx) for idx in
                             range(len(config['data_loader']))]
    else:
        raise ValueError("Check data_loader config, not correct format.")

    return data_loader, valid_data_loader, test_data_loader
