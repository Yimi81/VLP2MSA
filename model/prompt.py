...
class PromptGeneratorLayer(nn.Module):
    ...
    def forward(self, x, y):
        q = k = v = self.norm1(x)
        x = x + self.cross_attn(q, y, y)
        x = x + self.dropout(self.mlp(self.norm3(x)))
        return x


class VideoTextPrompt(nn.Module):
    ...
    def forward(self, text, visual):
        B, N, C = visual.shape
        text = text.to(dtype=self.dtype) # fp16 compatibility
        visual = visual.to(dtype=self.dtype) # fp16 compatibility
