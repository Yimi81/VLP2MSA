...
class BaseTrainer:
    ...
    def train(self):
        ...
        _, results, truths = self._valid_epoch(bTest=True)
        
        dataset_name = self.config["data_loader"]["args"]["dataset_name"]
        
        if dataset_name == "SIMS" or dataset_name == "SIMS_V2":
            eval_sims_regression(results, truths, bTest=True)
        else:
            eval_mosi(results, truths, bTest=True)
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        if save_best:
            ...
            self.logger.info("Saving current best at: {} / model_best.pth ...".format(self.checkpoint_dir))
