...
class TextVideoDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.sliding_window_stride != -1 and self.split == 'test':
        ...
        if self.transforms is not None:
            imgs_origin = self.transforms(imgs)

        final_origin = torch.zeros([self.video_params['num_frames'], 3, self.video_params['input_res'],
                             self.video_params['input_res']])
        final_origin[:imgs_origin.shape[0]] = imgs_origin
        final_augment_res = final_origin

        if self.split == "train":
            final_augment = copy.deepcopy(imgs)
            final_augment = self.transforms(final_augment)
            final_augment_res = torch.zeros([self.video_params['num_frames'], 3, self.video_params['input_res'],
                             self.video_params['input_res']])
            final_augment_res[:final_augment.shape[0]] = final_augment
            
        meta_arr = {'raw_captions': caption, 'paths': rel_fp, 'dataset': self.dataset_name}
        # final_origin: torch.Size([4, 3, 224, 224]), text: 'scene from a popular party', meta:
        data = {'video': final_origin, 'video_aug': final_augment_res, 'text': caption, 'meta': meta_arr, 'label': label}
        return data
